...
1. Iterate through the array of members to create a hash of on-site team sizes. {key: teamName, value: teamSize}.

2. Turn this hash into an array via Object.entries() and sort it from max to min team sizes. Then use the indices to build a teamIndex hash. Biggest team corresponds to value 0, and so on.

3. Start building a (sorted) array of array. Each subarray represents a team. How?
  3a. Iterate through the memberArray
  3b. If the member is not absent, look up the member's teamName in the Hash created at step 2 in order to push the member to the correct subarray.
    let teams = []
